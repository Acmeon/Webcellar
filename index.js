...
async function isPathAccessAuthorized(fp) {
	try {
		await fs.access(fp);
		if ((await fs.stat(fp)).isDirectory()) return false;
		for (var glob of globs) if (path.matchesGlob(fp, glob)) return true;
	} catch {}
	return false;
}
console.log(`
...
var csps = [
	...
	"'unsafe-inline'",
	"'unsafe-eval'",
	...program.opts().contentSecurityPolicySources ?? []
];
var modes = program.opts().mode ?? [];
if (modes.length == 0) {
	if (!fss.existsSync(`${cwd}/.webcellar/dependencies/office-js/init.txt`)) modes.push("init");
	if (!fss.existsSync(`${cwd}/.webcellar/dependencies/build.txt`)) modes.push("build");
	modes.push("run");
}
console.log(`Modes:`);
...
if (modes.includes("deinit")) await officeAddinDebugging.stopDebugging(`${cwd}/.webcellar/manifest.xml`);
if (!(modes.includes("init") || modes.includes("run"))) process.exit();
var app = express();
var options = await officeAddinDevCerts.getHttpsServerOptions(365);
https.createServer(options, app).listen(29640);
var webcellarCookieToDir = /* @__PURE__ */ new Map();
...
app.use(async (req, res) => {
	...
	if (modes.includes("init")) {
		...
		if (req.path == "/") {
			if (req.body) {
				var officeUrls = req.body.filter((url) => !url.startsWith("https://localhost:29640"));
				var error = false;
				for (var url of officeUrls) try {
			...
			if (req.path == "/.webcellar/taskpane.html") req_path = path.join(cwd, "/.webcellar/taskpane-init.html");
		...
		if (req.path.startsWith("/.webcellar")) {
			...
			if (reqFilePath.endsWith(".xlsx.js") || reqFilePath.endsWith(".xlsx.ts")) {
				...
				try {
					for (var imp of esModuleLexer.parse(contents)[0]) if (imp.n?.endsWith(".xlsx.js") || imp.n?.endsWith(".xlsx.ts")) {
						var webcellarFilePath = path.resolve(path.dirname(reqFilePath), imp.n);
						if (await isPathAccessAuthorized(webcellarFilePath)) webcellarFilePaths.push(webcellarFilePath);
					}
				} catch {}
				...
				for (var webcellarFilePath of webcellarFilePaths) {
					...
					var webcellarDirCookieVal = req.cookies[webcellarDirCookieName] ?? "";
				...
				while (true) {
					if ((await Array.fromAsync(fs.glob(`${webcellarDirPath}/*.xlsx.{js,ts}`))).length > 0) break;
					var next = path.dirname(webcellarDirPath);
					if (next == webcellarDirPath) {
						webcellarDirPath = "";
						break;
					} else webcellarDirPath = next;
				}
				var webcellarDirCookieName = `Webcellar#${encodeURIComponent(webcellarDirPath)}`;
				var webcellarDirCookieVal = req.cookies[webcellarDirCookieName] ?? "";
